...
- Operational reliability:
  ...
  - explicit degrade-vs-fail behavior under budget pressure
- Observability:
...
pub trait TelemetrySink {
    fn record_scalar(&mut self, key: &'static str, value: f64);
}
...
- Event-time semantics:
  ...
  - configurable late-data policy: Reject | BufferWithinWindow | ReorderByTimestamp
...
- per update step: p_change, run-length mode/mean, alert flag, alert_reason, and processing latency.
...
Establish initial SLO envelopes on a documented reference machine (CPU model, RAM, OS, toolchain pinned):
...
- Balanced mode reproducibility: repeated run objective drift <= 1e-9 for fixed seed/config
- Strict mode reproducibility: identical breakpoint and score outputs across repeated runs on same machine/toolchain
